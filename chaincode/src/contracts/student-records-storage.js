...
class StudentRecordsStorage extends Contract {
  ...
  async createPersonStudent(ctx, fullName) {

    const person = new ClientIdentity(ctx.stub);
    ...
    return JSON.stringify(person.cert.subject, null, 2)
  }

  async getPersonStudent(ctx, studentEmail) {
  ...
  async addStudentsSubject(ctx, studentEmail, semesterNumber, subjectName ) {
    ...
    const record = await this.getPersonStudent(ctx, studentEmail);
